...
char** sh_split_line(char* line)
{
	...
	if(!token)
	{
		fprintf(stderr,"deva-sh: allocation error\n");
		exit(EXIT_FAILURE);
	}
...
int sh_youtube(char** args){
	...
	char command[1000]="xdg-open https://www.youtube.com/results?search_query=";
	char* search_query=args[1];
	strcat(command,search_query);
	char **args2=sh_split_line(command);
	return sh_launch(args2);
